...
- 查询并比较任意港股（`HKEX:`+代码，或中文/英文名称）的行情、相对估值与三大财务报表明细。
...
支持任意港股代码（如 `HKEX:0001` 长和、`HKEX:2020` 安踏、`HKEX:9633` 农夫山泉）。以下三只为快捷锚点，提供中文别名：
...
- **有限覆盖与启发式标注**：任意港股可查，但估值分位、财务报表明细依赖上游可用性；三大报表的"标准科目汇总"由通用中文关键词规则聚合，对金融/地产/REIT 等特殊报表结构可能漏项或错项，应以原始科目为准逐项核对。

## 数据与故障边界

行情、相对估值和三大财务报表明细由本地 Python 适配器调用 AKShare 及其上游公开接口。上游结构、限流或网络变化可能导致数据不可用；Alfred 会对连接错误/超时/HTTP 429/5xx 做指数退避重试，失败时返回结构化错误或降级信息，而不是伪造数据。全量港股列表按 24 小时 TTL 缓存，避免频繁拉取触发限流。做出投资决策前，请用交易所公告、公司财报和券商成交记录复核。
