...
import type { AnnualFinancials, DcfInput, DcfResult, DiagnosticEvent, InstrumentProfile, RelativeInput, RelativeResult, StrategyComparison, StrategyCondition, StrategyDraftInput, StrategyMetricReference } from "../domain/types";
...
export class MockAppBridge implements AppBridge {
  ...
  async saveDcf(_profileId: string, input: DcfInput) { const result = mockDcf(input); const inserted = this.dcf?.contentHash !== result.contentHash; this.dcf = result; return { inserted, result }; }
