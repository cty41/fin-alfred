...
export function ResearchDataEditor({ bridge, profileId, onClose }: { bridge: AppBridge; profileId: string; onClose: () => void }) {
  ...
  return <section className="research-editor">
    ...
    <div className="online-quote"><button type="button" disabled={bridge.mode !== "desktop"} onClick={() => void refreshOnlineQuote()}>从已配置提供器刷新行情</button><p>在线失败不会覆盖现有快照，也不会阻止人工兜底。</p></div>
