...
test("real Gateway serves the prototype and keeps MCP restricted", async ({ page, request }) => {
  ...
  await expect(page.getByText("HK$25.4", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("DCF Base", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("213,600 股")).toBeVisible();
  await expect(page.getByText("HK$395,000")).toBeVisible();
  await expect(page.getByText("已完成")).toBeVisible();
  await expect(page.getByRole("heading", { name: "小米集团-W 历史估值" })).toBeVisible();
  await expect(page.getByRole("img", { name: "Valuation history" })).toBeVisible();
  await page.getByRole("button", { name: "查看估值历史" }).click();
  await expect(page.getByRole("dialog", { name: "估值历史 — 年报重算快照" })).toBeVisible();
  await expect(page.getByRole("dialog", { name: "估值历史 — 年报重算快照" }).getByText("2024-12-31", { exact: true }).first()).toBeVisible();
  await page.getByRole("dialog", { name: "估值历史 — 年报重算快照" }).getByText("关闭", { exact: true }).click();
  ...
  await expect(page.getByText(/Base Case DCF Value is HK\$/)).toBeVisible();
  await page.getByRole("button", { name: "Relative Valuation" }).click();
  await expect(page.getByText(/Base Case Relative Value is HK\$/)).toBeVisible();
  await page.reload();
  await expect(page.getByRole("heading", { name: "Watchlist" })).toBeVisible();
